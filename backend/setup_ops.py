...
def add_class(name, campus_id, default_fee):
    ...
    c = conn.cursor()
    try:
        c.execute('INSERT INTO classes(name, campus_id, default_fee) VALUES(?,?,?)',
                  (name, campus_id, default_fee))
    except sqlite3.IntegrityError:
        c.execute('UPDATE classes SET default_fee=? WHERE name=? AND campus_id=?',
                  (default_fee, name, campus_id))
    conn.commit()
    conn.close()
